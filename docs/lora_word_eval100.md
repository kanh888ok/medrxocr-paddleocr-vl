...
`aug-light step512` 是当前公开词图结果。它在 600 张训练图上加入模糊、亮度和旋转扰动，共生成 1800 张增强图。
...
实拍子集仍使用 `LoRA step512` 结果。`aug-light` 在实拍 18 张上没有提升，因此不作为实拍结果。
...
| rank4 | 推理太慢，停止 |
| rank16 | 前 20 张略差，未扩大 |
| 重增强 rank8 | 前 100 张略差，未扩大 |
| hard-focus | 前 100 张略差，未扩大 |
